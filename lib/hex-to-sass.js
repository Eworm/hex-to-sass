...
import HexToSassView from './hex-to-sass-view';
import {
    CompositeDisposable
} from 'atom';
...
export default {
    ...
    activate() {
        ...
        this.subscriptions.add(atom.commands.add('atom-workspace', {
            'hex-to-sass:transform': () => this.transform()
        }));
    },
    ...
    transform() {
        ...
        if (editor = atom.workspace.getActiveTextEditor()) {
            ...
            for (let i = 0; i < color_names.length; i++) {
                ...
                rgb = this.rgb(color);
                hsl = this.hsl(color);
                color_names[i].push(rgb[0], rgb[1], rgb[2], hsl[0], hsl[1], hsl[2]);
            }
            ...
            //
            let result = /^#?([a-f\d]{2})([a-f\d]{2})([a-f\d]{2})$/i.exec(selection);
            ...
            if (result != null) {
                ...
                if (!!resultColor.reduce(function(a, b) {
                    ...
                    // A grey
                    let greyPercentage = Math.floor(100 - (100 / 255) * resultColor[0]);

                    // Replace the selection with the correct variable
                    editor.insertText('$color--gray-' + greyPercentage + ': #' + selection + ';')

                } else {

                    // A color
                    let n_match = this.name(selection);
                    let name = n_match[1];
    ...
    name(color) {
        ...
        for (let i = 0; i < color_names.length; i++) {
            ...
            if (color == "#" + color_names[i][0])
                return ["#" + color_names[i][0], color_names[i][1], true];

            //
            ndf1 = Math.pow(r - color_names[i][2], 2) + Math.pow(g - color_names[i][3], 2) + Math.pow(b - color_names[i][4], 2);
            ndf2 = Math.pow(h - color_names[i][5], 2) + Math.pow(s - color_names[i][6], 2) + Math.pow(l - color_names[i][7], 2);
            ndf = ndf1 + ndf2 * 2;
        ...
        return (cl < 0 ? ["#000000", "Invalid Color: " + color, false] : ["#" + color_names[cl][0], color_names[cl][1], false]);
    },
